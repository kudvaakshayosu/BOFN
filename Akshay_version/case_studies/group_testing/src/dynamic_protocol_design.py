...
def simple_simulation(group_sizes=[80,90,100,110], prevalence=0.01, resolved=0., seed=1, test_period = None, verbose=False):
    ...
    for t in range(T):
        ...
        state[t+1], loss[t] = node(state[t], group_sizes[t], test_period= int(np.round(test_period.detach().numpy())), verbose=verbose)
...
def node(state, group_size, costs = {'test' : .001, 'infections' : 1, 'quarantine' : .01}, test_period = None, verbose=False):
    ...
    doubling_time = 3.0 # Fixed value
    alpha = 2 ** (1 / doubling_time)
    
    if test_period is None:
        test_period = 14 # days between tests default
    ...
    loss = tests_per_person_overall * costs['test'] * (14/test_period) # This cost is adjusted for different test period, more $$ for more often tests
    ...
    loss += people_in_quarantine * costs['quarantine'] * (test_period/14) # This cost is adjusted for different time periods, people happier if out of quaratine quicker
    ...
    loss += new_infections * costs['infections'] #*(14/test_period) # New infection costs is assumed to be the same
