...
# -*- coding: utf-8 -*-
"""
Created on Mon Mar 11 12:33:42 2024
...
def simu_push(world, thing, robot, base, simulation_steps):
    ...
    for t in range(simulation_steps+100):
        ...
        if t == int(simulation_steps/3):
            output_val[...,[0,1]] = copy.deepcopy(torch.tensor(list(thing.position)))
            
        if t == int(2*simulation_steps/3):
             output_val[...,[2,3]] = copy.deepcopy(torch.tensor(list(thing.position)))         
                
        if t == simulation_steps - 1:
            rstop = True
         
        if t == simulation_steps:
            output_val[...,[4,5]] = copy.deepcopy(torch.tensor(list(thing.position)))
